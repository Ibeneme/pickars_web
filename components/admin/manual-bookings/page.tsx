...
import LoadingScreen from "@/components/atoms/loaders/loading-screen";

export default function ManualRideDispatch() {
  ...
  const statusCounts = useMemo(() => {
    ...
    allRides.forEach((ride: any) => {
      ...
      else if (ride?.isManual && !ride?.acceptRide) status = "Awaiting";
  ...
  const handleViewRide = (ride: any) => {
    if (ride?.trackingId) {
      router.push(`/admin/manual-booking/${ride.trackingId}`);
    }
  };
        ...
        <div className="flex flex-col gap-8">
          <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={() => router.push("/admin/manual-booking/create")}
