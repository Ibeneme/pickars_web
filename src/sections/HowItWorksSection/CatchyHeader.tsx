import React from "react";
import { motion, type Variants } from "framer-motion";
import woman_and_phone from "../../assets/images/about/phone_in_pocket.jpeg";
import photo from "../../assets/images/use_app/nice.svg";
import DownloadButtons from "../../components/buttons/DownloadButtons";

// Animation variants for cleaner code and staggered reveals
const fadeUpVariant: Variants = {
  hidden: { opacity: 0, y: 40 },
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.6, ease: "easeOut" },
  },
};

const scaleInVariant: Variants = {
  hidden: { opacity: 0, scale: 0.95 },
  visible: {
    opacity: 1,
    scale: 1,
    transition: { duration: 0.7, ease: "easeOut" },
  },
};

export const CatchyHeader: React.FC = () => {
  return (
    <section className="py-16 sm:py-20 md:py-24 bg-[#fff]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-6 md:space-y-8">
        {/* Top Grid: Catchy Header Copy & Image */}
        <div className="grid md:grid-cols-2 gap-6 md:gap-8 items-center">
          {/* Left Column: Catchy Headline & Subtext */}
          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, margin: "-100px" }}
            variants={fadeUpVariant}
            className="relative bg-black rounded-[32px] sm:rounded-[40px] p-6 sm:p-8 md:p-12 flex flex-col justify-between overflow-hidden min-h-[500px] sm:min-h-[680px]"
          >
            <div className="my-6">
              <div
                className="mb-2 mt-4 sm:mb-3 px-5 sm:px-6 py-2 sm:py-2.5 bg-[#FF0000] text-white font-black text-[11px] sm:text-xs md:text-sm uppercase tracking-widest cursor-default select-none w-fit inline-block"
                style={{
                  maskImage:
                    "radial-gradient(circle 5px at calc(100% - 2.5px) 50%, #0000 99%, #000 100%)",
                  WebkitMaskImage:
                    "conic-gradient(from -45deg at 50% 50%, #000 0 90deg, #0000 0) 0 0/10px 10px repeat",
                }}
              >
                FAST & RELIABLE DELIVERY
              </div>

              <h1 className="text-4xl sm:text-6xl md:text-7xl font-black text-white tracking-tight leading-[1.05]">
                Your package, <br />
                on time.
              </h1>
              <p className="mt-5 text-base sm:text-lg font-normal leading-relaxed text-zinc-300">
                Skip the dispatch headaches. With Pickars, your deliveries
                across Port Harcourt are fast, fully tracked, and handled by
                elite professionals.
              </p>
            </div>

            {/* App Download Buttons */}
            <DownloadButtons dark={true} alignLeft={true} />
          </motion.div>

          {/* Right Column: Featured Image */}
          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, margin: "-100px" }}
            variants={scaleInVariant}
            className="relative overflow-hidden rounded-[32px] sm:rounded-[40px] min-h-[500px] sm:min-h-[680px] h-full"
          >
            <img
              src={woman_and_phone}
              alt="Delivery experience with Pickars"
              className="w-full h-full object-cover absolute inset-0"
            />
          </motion.div>
        </div>

        {/* Bottom Full-Width Showcase Grid Item */}
        <motion.div
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }}
          variants={fadeUpVariant}
          className="relative overflow-hidden rounded-[32px] sm:rounded-[40px] w-full max-h-[600px]"
        >
          <img
            src={photo}
            alt="Pickars service showcase"
            className="w-full h-auto object-cover block"
          />

          {/* Optional Gradient Overlay for better text readability */}
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />

          {/* Pickars Text Overlay */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.2 }}
            viewport={{ once: true }}
            className="absolute bottom-6 left-6 sm:bottom-10 sm:left-10 z-10"
          >
            <h3 className="text-white text-7xl sm:text-9xl font-black tracking-tighter font-['Lufga']">
              Pickars<span style={{ color: "#ff0000" }}>.</span>
            </h3>
          </motion.div>
        </motion.div>
      </div>
    </section>
  );
};
