...
export default function DownloadButtons({
  ...
  const buttonStyles = `flex min-w-[180px] items-center justify-center gap-3 rounded-[120px] px-3 py-2 transition-all ${
    dark
      ? "bg-white text-black hover:bg-[#FF0000] hover:text-white"
      : "bg-black text-white hover:bg-[#FF0000]"
  }`;

  const subtitleStyles = `text-[10px] font-semibold uppercase tracking-wider ${
    dark ? "text-zinc-500" : "text-zinc-400"
  }`;

  // Determines alignment: centered on mobile, left-aligned on web if alignLeft is true
  const alignmentClass = alignLeft
    ? "justify-center md:justify-start"
    : "justify-center";

  return (
    <div className={`mt-0 flex flex-wrap items-center gap-4 ${alignmentClass}`}>
      ...
      <a
        ...
        className={buttonStyles}
      >
        <svg className="h-6 w-6 shrink-0 fill-current" viewBox="0 0 512 512">
